...
class TenantEventStreamTest {
    ...
    fun `create happy path`() {
        val eventSlot = slot<TenantV1Schema>()
        ...
        assertThat(result.id).isEqualTo(tenantId.value)
    ...
    fun `delete happy path`() {
        every { eventHandler.delete(tenantId) } just runs
        inputTopic.pipeInput("tenant/$tenantId", deleteEvent)
        verify(exactly = 1) { eventHandler.delete(tenantId) }
        assertThat(outputTopic.isEmpty).isTrue()
    }
